...
\begin{proofs}
	...
	+\frac{1}{2!}X\tp \mbome{H_f(X_0)} X\cdot t^2+o(\|tX\|^2)
	\end{equation*}
	注意到$f(X_0+t X)\ge f(X_0)$，于是
	$0\le \dfrac{1}{2!}X\tp H_f(X_0) X\cdot  t^2+ t^2 o(1)$，
	进而可得知$0\le X\tp H_f(X_0) X$。这表明$H_f(X_0)$
	为正定或半正定。
\end{proofs}
...
我们已知，取$P_0(x_0，y_0，z_0)\in \mathbb{R}^3$，设 $\v{e}=(a，b，c)\tp \in\mathbb{R}^3$为非零向量，过$P_0$沿方向$\v{e}$的直线$\varGamma$的方程为
\begin{equation*}
...
过$P_0$并且与$\varGamma$垂直的平面$S$称为$\varGamma$过$P_0$的\tboba{法平面}，它的方程为
$\begin{pmatrix}
	x-x_0\\[-3pt]
	y-y_0\\[-3pt]
	z-z_0
...
我们称$\v{e}$为平面$S$的\tboba{法向量}，$\varGamma$为$S$的\tboba{法线}。
...
考虑$\varGamma: \begin{cases}
	x=x(t)，\\[-3pt]
	y=y(t)，\\[-3pt]
	z=z(t)，
\end{cases}
~~t\in [\alpha，\beta]$，
若上述这些函数在点$t=t_0$处可微，则称曲线$\varGamma$在
...
\end{equation*}
我们将经过点$P_0$并且与上述切线垂直的平面称为$\varGamma$在点$P_0$处的\tboba{法平面}，其方程为
\begin{equation*}
...
考虑$\varGamma: \begin{cases}
	...
	F_2(x，y，z)=0，
\end{cases}$~~
设$F_1，F_2$在点$P_0(x_0，y_0，z_0)$可微 且$\mathop\mathrm{grad}F_1(P_0)$，$\mathop\mathrm{grad}F_2(P_0)$不为零，则曲线$\varGamma$在该点的切线为
\begin{equation*}
	\begin{cases}
...
\begin{example}
	求$\displaystyle\oint_{L^{+}}\frac{x\dif x+y\dif y+z\dif z}{x^2+y^2+z^2}$，其中曲线$L$为球面$S: x^2+y^2+z^2=a^2$在第一卦限中与坐标平面
	相交的圆弧$\wideparen{AB}$，$\wideparen{BC}$，
	$\wideparen{CA}$连接而成的闭曲线。
	\begin{solution}
		记球面\(S\)的第一卦限部分为\(S'\)，定义其正向背离原点，则\(L^+=\partial S'^+\)。由Stokes公式可知
		\begin{equation*}
			\oint_{L^{+}}\frac{x\dif x+y\dif y+z\dif z}{x^2+y^2+z^2}
			=\frac{1}{a^2}\oint_{L^{+}}x\dif x+y\dif y+z\dif z
			=\frac{1}{a^2}\iint_{S'^{+}}\v{\nabla}\times (x，y，z)\tp\cdot \dif \v{\sigma}
			=0 \qedhere
		\end{equation*}
	\end{solution}
\end{example}
\begin{example}
	...
	\begin{solution}
		...
		\begin{equation*}
			\oint_{L^{+}}\frac{x\dif y-y\dif x}{x^2+y^2}
			=\iint_{S^{+}}\v{\nabla}\times \Big(\frac{-y}{x^2+y^2}，\frac{x}{x^2+y^2}，0\Big)\tp \cdot \dif \v{\sigma}
			=0
		\end{equation*}

		（2）以$L^{+}$（逆时针）为准线、$z$轴为母线作一个柱面，记
		该柱面与$xy$平面的交线为$L_1^{+}$（逆时针），该柱面的侧面
		为$S^+$，其正向向外，则知\(\partial S^+ = L^- + L_1^+\)，由Stokes公式可知
		\begin{equation*}
			\oint_{L^{-}+ L_1^{+}}\frac{x\dif y-y\dif x}{x^2+y^2}
			=\iint_{S^{+}}\v{\nabla}\times\Big(\frac{-y}{x^2+y^2}，\frac{x}{x^2+y^2}，0\Big)\tp \cdot \dif \v{\sigma}
			=0
		\end{equation*}
		即有$\displaystyle \oint_{L^{+}}\frac{x\dif y-y\dif x}{x^2+y^2}=\oint_{L_1^{+}}\frac{x\dif y-y\dif x}{x^2+y^2}$。设$L_1$在$xy$平面所围区域为$D$，取$\delta>0$使得$B((0，0); \delta)\subset D$，令$L_2=\partial B((0，0); \delta)$，则由Green公式可得
		\begin{equation*}
			\oint_{L_1^++L_2^-} \frac{x\dif y-y\dif x}{x^2+y^2}
			=\iint_{D \setminus B((0，0); \delta)} \left(\dfrac{\partial}{\partial x}\frac{x}{x^2+y^2}+\dfrac{\partial}{\partial y}\frac{y}{x^2+y^2}\right) \dif x \dif y
			=0
		\end{equation*}
		于是
		\begin{equation*}
			\oint_{L^{+}}\frac{x\dif y-y\dif x}{x^2+y^2}
			=\oint_{L_1^{+}}\frac{x\dif y-y\dif x}{x^2+y^2}
			=\oint_{L_2^{+}}\frac{x\dif y-y\dif x}{x^2+y^2}
			=2\pi	\qedhere
		\end{equation*}
	\end{solution}
\end{example}
...
\subsubsection{平面上第二类曲线积分与路径的无关性}

\di[平面上第二类曲线积分仅依赖始末点的充要条件]{
...
\begin{proofs}
	\adjline
	假设$L_1，L_2\subset \varOmega$ 为连接$A，B$ 的两条分段
	光滑的曲线，从$A$出发经$L_1$到$B$后， 再沿$L_2$
	回到$A$可得到过$A，B$的分段光滑的闭曲线$\varGamma$；
	而由过$A，B$的任意分段光滑闭曲线$\varGamma$， 也可以切分
	得到连接$A，B$的分段光滑曲线$L_1，L_2$。此时
	\begin{equation*}
	\oint_{\varGamma^{+}}\v{F}\cdot \dif \v{\ell}
	=\int_{L_1(A)}^{(B)}\v{F}\cdot \dif \v{\ell}-\int_{L_2(A)}^{(B)}\v{F}\cdot \dif \v{\ell}
	\end{equation*}
	则$\displaystyle \oint_{\varGamma^{+}}\v{F}\cdot \dif \v{\ell}=0 \Longleftrightarrow \int_{L_1(\wideparen{AB})}\v{F}\cdot \dif \v{\ell}=\int_{L_2(\wideparen{AB})}\v{F}\cdot \dif \v{\ell}$，
	因此所证结论成立。	
\end{proofs}
...
\di[平面单连通开区域上第二类曲线积分仅依赖始末点的充要条件]{
	假设$\varOmega\subset\mathbb{R}^2$为\tbome{单连通}开区域，而函数
	$\v{F}=(F_1，F_2)\tp$在$\varOmega$上连续可导，则下列结论等价：
	...
	（3）存在函数$U:\varOmega\rightarrow\mathbb{R}$，使得$\forall (x，y)\in \varOmega$，
	\(\dif U(x，y)=F_1(x，y)\dif x+F_2(x，y)\dif y\)。
}
\begin{proofs}
	\textbf{（1）\(\Rightarrow\)（2）\quad}对于$\varOmega$中过$A，B$的分段光滑闭曲线$L$，设其所围区域为$\varOmega_1$，由Green公式知
	\begin{equation*}
	\oint_{L^{+}}\v{F}\cdot \dif \v{\ell}
	=\oint_{L^{+}} F_1\dif x+F_2\dif y
	=\iint_{\varOmega_1}\left(\frac{\partial F_2}{\partial x}-\frac{\partial F_1}{\partial y}\right)\dif x\dif y
	=0
	\end{equation*}
	进而由前面定理~\ref{平面上第二类曲线积分仅依赖始末点的充要条件}~可知（2）成立。
	
	\textbf{（2）\(\Rightarrow\)（3）\quad}固定$A\in \varOmega$，$\forall B=(x_0，y_0)\in \varOmega$，  构造
	\(U(x_0，y_0)=\dint_{L(AB)}F_1\dif x+F_2\dif y\)，
	下面证明$\dif U=F_1\dif x+F_2\dif y$。
	...
	\textbf{（3）\(\Rightarrow\)（1）\quad}由于$\dif U=F_1\dif x+F_2\dif y$，
	\(F_1=\dfrac{\partial U}{\partial x}，F_2=\dfrac{\partial U}{\partial y}\)，
	于是$\dfrac{\partial^2U}{\partial y\partial x}=\dfrac{\partial F_1}{\partial y}，
	\dfrac{\partial^2U}{\partial x\partial y}=\dfrac{\partial F_2}{\partial x}$。
	又$F_1，F_2$连续可导，  因此$\dfrac{\partial^2U}{\partial y\partial x}，\dfrac{\partial^2U}{\partial x\partial y}$ 均连续，
	即$\dfrac{\partial^2U}{\partial y\partial x}=\dfrac{\partial^2U}{\partial x\partial y}$，也即$\frac{\partial F_1}{\partial y}=\frac{\partial F_2}{\partial x}$，故（1）成立。
\end{proofs}

这个定理是说，单连通区域$\varOmega\subset\mathbb{R}^2$上的微分形式$F_1\dif x + F_2\dif y$ 具有原函数，当且仅当$\dfrac{\partial F_1}{\partial y}=\dfrac{\partial F_2}{\partial x}$。此外，显然若$U$为$F_1\dif x+F_2\dif y$的一个原函数，那么$U+C$也是上述微分形式的原函数，
其中$C$为任意的常数。

\begin{theorem}
	假设$\varOmega\subset\mathbb{R}^2$为单连通开区域，而函数 $\v{F}=(F_1，F_2)\tp$ 在$\varOmega$ 上连续且使得$F_1\dif x+F_2\dif y$在$\varOmega$上有原函数$U$，则$\forall A=(x_1，y_1)，B=(x_2，y_2)\in \varOmega$，
	\begin{equation*}
	\int_{L(AB)}F_1\dif x+F_2\dif y=U(x_2，y_2)-U(x_1，y_1)
	=U(B)-U(A)=U\Big|_{(x_1，y_1)}^{(x_2，y_2)}
	\end{equation*}
\end{theorem}
...
\begin{example}
	...
	\begin{proofs}
		...
		注意到
		\begin{equation*}
		\dfrac{\partial}{\partial y}\left(\ln y-\dfrac{y}{x}\right)=\dfrac{1}{y}-\dfrac{1}{x}，\qquad
		\dfrac{\partial}{\partial x}\left(\dfrac{x}{y}-\ln x\right)=\dfrac{1}{y}-\dfrac{1}{x}
		\end{equation*}
...
\subsubsection{有势场和势函数}

\de[有势场、势函数]{
	设有定义在区域 $D\subset \mathbb{R}^3$ 上的向量场 $\v{F}=(P，Q，R)$。
	如果存在 $D$ 上的数量场 $\phi$ ，满足 $\forall \v{p}=(x，y，z)\in D$，
	$ \nabla \phi (p)= \mathop{\mathrm{grad}}\phi (p)=\v{F}(\v{p})$，
	那么就称 $\v{F}$ 是\tboba{有势场}，$\phi$ 成为 $\v{F}$ 的一个\tboba{势函数}。
	
	等价地，势函数满足
	$\dfrac{\partial \phi}{\partial x}=P，\dfrac{\partial \phi}{\partial y}=Q，\dfrac{\partial \phi}{\partial z}=R$。
}
...
\di[有势场保守无旋]{
	...
	\begin{definition}
		如果 
		$ \mathop{\mathrm{rot}}\v{F}=\v{\nabla} \times \v{F}=\begin{pmatrix}
			\dfrac{\partial R}{\partial y} - \dfrac{\partial Q}{\partial z} &
			\dfrac{\partial P}{\partial z} - \dfrac{\partial R}{\partial x} &
			\dfrac{\partial Q}{\partial x} - \dfrac{\partial P}{\partial y}
		\end{pmatrix}\tp=\v{0}$
		在$D$ 上处处成立，则称$\v{F}$ 为 $D$ 上的一个\textbf{无旋场}。
	\end{definition}
	
	（3）\(\v{F}\)是保守场。
	\begin{definition}
		如果对含于 $D$ 中的任何一条封闭曲线 $\Gamma$，都有 $ \dint_{\Gamma} \v{F}\cdot \dif \v{p}=0$，
		则称 $\v{F}$ 为 $D$ 上的一个\textbf{保守场}。
	\end{definition}
}
\begin{proofs}
	\adjline
	\textbf{（1）\(\Rightarrow\)（2）\quad}假设 $\v{F}=(P，Q，R)$ 是有势场，$\phi$ 是 $\v{F}$ 的一个势函数，即知$\dfrac{\partial \phi}{\partial x}=P，\dfrac{\partial \phi}{\partial y}=Q，\dfrac{\partial \phi}{\partial z}=R$，于是
	\begin{align*}
		\frac{\partial R}{\partial y}=\frac{\partial^2\phi}{\partial y \partial z}=\frac{\partial^2\phi}{\partial z \partial y}=\frac{\partial Q}{\partial z}，\qquad
		\frac{\partial P}{\partial z}=\frac{\partial^2\phi}{\partial z\partial x}=\frac{\partial^2\phi}{\partial x\partial z}=\frac{\partial R}{\partial x}，\qquad
	...
	\textbf{（2）\(\Rightarrow\)（3）\quad}在$D$中任取一条封闭曲线$\Gamma$，并且 在 $D$ 内作一个以$\Gamma$
	为边界的曲面$\varSigma$，由 Stokes 公式就有
	$$ \int_\Gamma \v{F}\cdot \dif{\v\ell}= \int_{\varSigma} \mathop{\rm rot} \v{F}\cdot \dif {\v{\sigma}}=0$$
	从而  $\v{F}$ 是 $D$ 上的保守场。

	\textbf{（3）\(\Rightarrow\)（1）\quad}$\v{F}$ 的曲线积分与路径无关，即若 $\Gamma_1，\Gamma_2$ 是两条有向曲线，它们都以 $A$ 为起点，$B$ 为终点.  则
	$ \dint_{\Gamma_1} \v{F} \cdot \dif \v{\ell}= \int_{\Gamma_2} \v{F} \cdot \dif \v{\ell}$。固定 $A=(a，b，c)$，而 $B=(x，y，z) $ 是 $D$中的任意一（变化）点，定义函数
	$$\phi(x，y，z)=\int_A^B  \v{F}\cdot \dif  \v{\ell}=\int_{(a，b，c)}^{(x，y，z)}  \v{F}\cdot \dif \v{\ell}$$
	取\(h\)使得$(x+h，y，z)$ 仍然在$D$ 内，令 $B'=(x+h，y，z)$ ，则 
	$\phi(x+h，y，z)=\dint_A^{B'}  \v{F}\cdot \dif \v{\ell}$，
	于是有积分中值定理
	\begin{equation*}
		\phi(x+h，y，z)-\phi(x，y，z)
		= \int_B^{B'}  \v{F}\cdot \dif \v{\ell} 
		=\int_x^{x+h} P(t，y，z) \dif t \xlongequal{\exists x^\ast \in (x，x+h)} hP(x^\ast，y，z)
	\end{equation*}
...
\begin{proofs}[\yan （\textbf{1}）的证明]
	把$\Gamma(s)$分成两部分：
	$$\Gamma(s)=\int_0^1t^{s-1}\mathrm{e}^{-t}\dif t+\int_1^{+\infty}t^{s-1}\mathrm{e}^{-t}\dif t$$
	对于任意$\beta>\alpha>0$，让$s\in[\alpha，\beta]$，则当$0<t<1$时，
	$0<t^{s-1}\mathrm{e}^{-t}\le t^{\alpha-1}\mathrm{e}^{-t}$，
	因为$\dint_0^1t^{a-1}\mathrm{e}^{-t}\dif t$收敛，所以$\dint_0^1t^{s-1}\mathrm{e}^{-t}\dif t$关于$s$在$[\alpha，\beta]$上一致收敛，因而
	$\int_0^1t^{s-1}e^{-t}\dif t$ 是$[\alpha，\beta]$上的连续函数。
	
	当$t>1$时，
	$t^{s-1}\mathrm{e}^{-t}\le t^{\beta-1}\mathrm{e}^{-t}$，
	因为$\dint_1^+\infty i^{\beta-1}\mathrm{e}^{-t}\dif t$ 收敛，所以积分$\dint_1^+\infty t^{s-1}\mathrm{e}^{-t}\dif t$ 关于$s$ 在$[\alpha，\beta]$上一致收敛，因而$\dint_1^{+\infty}t^{s-1}e^{-t}\dif t$是$[\alpha，\beta]$上的连续函数。
...
\end{theorem}
\begin{theorem}
	\textbf{\textup{余元公式\quad}}对任意\(p\in(0，1)\)，有\(\varGamma(p)\varGamma(1-p) = \dfrac{\pi}{\sin p\pi}\)。
\end{theorem}
...
\begin{theorem}
	\textup{Beta函数}也可表示为\(B(p，q) = \dint_0^{+\infty} \dfrac{x^{q-1}}{(1+x)^{p+q}} \dif x\)。
\end{theorem}
...
\begin{example}
	...
	\begin{solution}
		...
		\begin{equation*}
			\dint_0^\frac{\pi}{2} \cos^\alpha x \sin^\beta x \dif x = \dfrac{1}{2} \dint_0^1 (1-t)^\frac{\alpha-1}{2} t^\frac{\beta-1}{2}\dif t = \dfrac{1}{2} B\left(\dfrac{\beta+1}{2}，\dfrac{\alpha+1}{2}\right) = \dfrac{\varGamma\left( \dfrac{\beta+1}{2} \right) \varGamma\left( \dfrac{\alpha+1}{2} \right)}{2\varGamma\left( \dfrac{\alpha+\beta}{2}+1 \right)} \qedhere
		\end{equation*}
	\end{solution}
